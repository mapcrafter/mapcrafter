...
IsometricTileSet::IsometricTileSet(const mc::World& world)
	: min_depth(0), depth(0) {
	scan(world);
}
...
void IsometricTileSet::findRenderTiles(const mc::World& world, bool auto_center,
		TilePos& tile_offset) {
...
void IsometricTileSet::findRequiredCompositeTiles(const std::set<TilePos>& render_tiles,
...
void IsometricTileSet::updateContainingRenderTiles() {
...
void IsometricTileSet::scan(const mc::World& world) {
	TilePos tile_offset(0, 0);
	scan(world, false, tile_offset);
	setDepth(min_depth);
}

void IsometricTileSet::scan(const mc::World& world, bool auto_center, TilePos& tile_offset) {
	findRenderTiles(world, auto_center, tile_offset);
	setDepth(min_depth);
}

void IsometricTileSet::scanRequiredByTimestamp(int last_change) {
	required_render_tiles.clear();
...
void IsometricTileSet::scanRequiredByFiletimes(const fs::path& output_dir,
...
int IsometricTileSet::getMinDepth() const {
	return min_depth;
}

int IsometricTileSet::getDepth() const {
	return depth;
}

void IsometricTileSet::setDepth(int depth) {
...
const TilePos& IsometricTileSet::getTileOffset() const {
	return tile_offset;
}

bool IsometricTileSet::hasTile(const TilePath& path) const {
	if (path.getDepth() == depth)
		return render_tiles.count(path.getTilePos()) != 0;
	return composite_tiles.count(path) != 0;
}

bool IsometricTileSet::isTileRequired(const TilePath& path) const {
	if(path.getDepth() == depth)
		return required_render_tiles.count(path.getTilePos()) != 0;
	return required_composite_tiles.count(path) != 0;
}

int IsometricTileSet::getRequiredRenderTilesCount() const {
	return required_render_tiles.size();
}

const std::set<TilePos>& IsometricTileSet::getRequiredRenderTiles() const {
	return required_render_tiles;
}

int IsometricTileSet::getRequiredCompositeTilesCount() const {
	return required_composite_tiles.size();
}

const std::set<TilePath>& IsometricTileSet::getRequiredCompositeTiles() const {
	return required_composite_tiles;
}

int IsometricTileSet::getContainingRenderTiles(const TilePath& tile) const {
