...
class LightLevelOverlay : public TintingOverlay<config::LightLevelOverlaySection> {
public:
	LightLevelOverlay(std::shared_ptr<config::ConfigSection> overlay_config);

protected:
	virtual RGBAPixel getBlockColor(const mc::BlockPos& pos, const mc::BlockPos& pos_for, uint16_t id, uint16_t data);

	bool day;
	std::array<RGBAPixel, 16> colors;
};

}
}

#endif /* RENDERMODES_LIGHTLEVELOVERLAY_H_ */
