...
std::ostream& operator<<(std::ostream& out, BlockHandlerType type) {
	if (type == BlockHandlerType::DEFAULT)
		out << "default";
	else if (type == BlockHandlerType::CAVE)
		out << "cave";
	else
		assert(false);
	return out;
...
AbstractBlockHandler::AbstractBlockHandler()
	: use_preblit_water(false), images(nullptr), world(nullptr) {
}

void AbstractBlockHandler::initialize(const RenderView* render_view, BlockImages* images,
...
void AbstractBlockHandler::setUsePreblitWater(bool use_preblit_water) {
...
void AbstractBlockHandler::handleBlock(const mc::BlockPos& pos, uint16_t& id, uint16_t& data) {
	...
	if (id == 2) { // grass blocks
		...
		*/
		...
		if (data >= 8) {
			...
			data = getBlock(pos + mc::DIR_BOTTOM).data | LARGEPLANT_TOP;
		}
	}
...
mc::Block AbstractBlockHandler::getBlock(const mc::BlockPos& pos, int get) {
	return world->getBlock(pos, *current_chunk, get);
}

CaveBlockHandler::CaveBlockHandler(const std::vector<mc::BlockPos>& hidden_dirs)
	: hidden_dirs(hidden_dirs) {
}

void CaveBlockHandler::handleBlock(const mc::BlockPos& pos, uint16_t& id, uint16_t& data) {
	if (isHidden(pos, id, data))
		id = 0;
}

bool CaveBlockHandler::isHidden(const mc::BlockPos& pos, uint16_t id, uint16_t data) {
...
bool CaveBlockHandler::isLight(const mc::BlockPos& pos) {
	return getBlock(pos, mc::GET_SKY_LIGHT).sky_light > 0;
}

bool CaveBlockHandler::isTransparentBlock(const mc::Block& block) const {
...
BlockHandler* createBlockHandler(const config::MapSection& map_config) {
	BlockHandlerType type = map_config.getBlockHandler();
	if (type == BlockHandlerType::DEFAULT) {
		return new AbstractBlockHandler();
	} else if (type == BlockHandlerType::CAVE) {
		...
		if (map_config.getRenderView() == RenderViewType::ISOMETRIC)
			hidden_dirs = {mc::DIR_SOUTH, mc::DIR_WEST, mc::DIR_TOP};
		else
			hidden_dirs = {mc::DIR_TOP};
		return new CaveBlockHandler(hidden_dirs);
	}
