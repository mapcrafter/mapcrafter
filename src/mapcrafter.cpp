...
int main(int argc, char** argv) {
	...
	all.add_options()
		...
		("render-skip,s", po::value<std::vector<std::string>>(&render_skip),
			"skips rendering the specified map(s)")
		("render-reset,r", "skips rendering all maps")
		("render-auto,a", po::value<std::vector<std::string>>(&render_auto),
			"renders the specified map(s)")
		("render-force,f", po::value<std::vector<std::string>>(&render_force),
